...
def carbon(code):
    ...
    for func in funcs:
        if func.name == "__init__":
            continue
        newname = "".join(random.choice(["I", "l"])
    ...
    for i in range(len(originals)):
        for key in pairs:
            originals[i] = re.sub(
                r"({.*)(" + key + r")(.*})", "\\1" + pairs[key] + "\\3", originals[i], re.MULTILINE)
...
def oxyry(code):
    try:
        ...
        payload = {
            ...
            "rename_nondefault_parameters": False,
            "rename_default_parameters": False,
            "preserve": "Aimbot Inter KeyValidator",
            "source": src
        }
...
def anubis(code):
    newcode = "\n"
    classes = ["".join(random.choice(string.ascii_lowercase + string.ascii_uppercase)
                       for i in range(random.randint(8, 20))) for i in range(random.randint(2, 5))]
    for i in classes:
        newcode += f"class {i}:\n    def __init__(self):\n"
        funcs = ["__"+"".join(random.choice(string.ascii_lowercase + string.ascii_uppercase)
                              for i in range(random.randint(8, 20))) for i in range(random.randint(5, 15))]
        for i in funcs:
            newcode += f"        self.{i}()\n"
        for i in funcs:
...
parser = argparse.ArgumentParser(description="Description of your anubis")

# 添加命令行参数
parser.add_argument('--coustom', type=bool,
                    default=False, help='coustom params')
...
coustom = args.coustom
files = args.files

# while True:
# if len(files) == 0:
#     file_paths = input(purple(
#         "        [>] Enter the python file you wish to obfuscate [script.py] : ") + "\033[38;2;148;0;230m")
...
if coustom == True:
    while True:
        ans = input(
            purple("        [>] AntiDebug [y/n] : ") + "\033[38;2;148;0;230m").lower()
        if ans == "y":
            bug = True
            break
        elif ans == "n":
    ...
    while True:
        ans = input(
            purple("        [>] Junk Code [y/n] : ") + "\033[38;2;148;0;230m").lower()
        if ans == "y":
            junk = True
            break
        elif ans == "n":
    ...
    if rename:
        while True:
            ans = input(purple(
                "        [>] Carbon (Offline) or Oxyry [c/o] : ") + "\033[38;2;148;0;230m").lower()
            if ans == "c":
                carbonate = True
                break
            elif ans == "o":
    ...
    while True:
        ans = input(purple(
            "        [>] One Line Obfuscation (Can't compile to exe) [y/n] : ") + "\033[38;2;148;0;230m").lower()
        if ans == "y":
            extra = True
            break
        elif ans == "n":
...
for file in files:
    ...
    print(
        blue(f"        [>] Code has been successfully obfuscated @ {name}"), end="")

# if extra == False:
#     compile = False
#     while True:
#         ans = input(purple(
#             "        [>] Would you like to compile to an exe [y/n] : ") + "\033[38;2;148;0;230m").lower()
#         if ans == "y":
#             compile = True
#             break
#         elif ans == "n":
...
#     if compile == True:
#         basic_params = ["nuitka", "--mingw64", "--onefile", "--enable-plugin=numpy",
#                         "--include-module=psutil", "--remove-output", "--assume-yes-for-downloads", name]
#         p = subprocess.Popen(
#             basic_params, stdout=subprocess.DEVNULL, shell=True, cwd=os.getcwd())
#         print(red(
